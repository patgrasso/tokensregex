...
ops['ATTR'] = (stack, attr, value) => {
  let s;

  if (value instanceof RegExp) {
    s = State((tok) => value.test(tok[attr]), false, value);
  } else if (typeof value === 'string') {
    s = State((tok) => tok[attr] === value);
  }
  stack.push(Frag(s, [ s.out ]));
};
...
module.exports = { post2NFA, simulateNFA, EPS };
